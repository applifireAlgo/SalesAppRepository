Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.SalesRegionMainController', {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#SalesRegion');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#SalesRegionTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#SalesRegionTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#SalesRegion');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SalesRegion').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#SalesRegion').getViewModel().setData(responseData.response.data);
     },
