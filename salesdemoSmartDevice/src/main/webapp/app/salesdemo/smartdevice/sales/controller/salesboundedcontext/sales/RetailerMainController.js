Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.RetailerMainController', {
     ...
     distributorcodeLoad: function() {
          ...
          Ext.Ajax.request({
               url: AppRestUrl + 'secure/Distributor/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Retailer');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#RetailerTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#RetailerTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Retailer');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Retailer').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#Retailer').getViewModel().setData(responseData.response.data);
     },
