Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.MaterialMainController', {
     ...
     brandcodeLoad: function() {
          ...
          Ext.Ajax.request({
               url: AppRestUrl + 'secure/Brand/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Material');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#MaterialTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#MaterialTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Material');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Material').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#Material').getViewModel().setData(responseData.response.data);
     },
