Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.ChannelMainController', {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Channel');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#ChannelTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#ChannelTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Channel');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Channel').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#Channel').getViewModel().setData(responseData.response.data);
     },
