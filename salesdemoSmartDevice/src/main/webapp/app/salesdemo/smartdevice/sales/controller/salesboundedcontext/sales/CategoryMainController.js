Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.CategoryMainController', {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Category');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#CategoryTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CategoryTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Category');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Category').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#Category').getViewModel().setData(responseData.response.data);
     },
