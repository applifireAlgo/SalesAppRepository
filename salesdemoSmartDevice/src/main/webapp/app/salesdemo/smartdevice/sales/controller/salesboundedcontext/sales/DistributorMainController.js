Ext.define('Salesdemo.salesdemo.smartdevice.sales.controller.salesboundedcontext.sales.DistributorMainController', {
     ...
     regioncodeLoad: function() {
          ...
          Ext.Ajax.request({
               url: AppRestUrl + 'secure/SalesRegion/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Distributor');
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#DistributorTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#DistributorTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Distributor');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Distributor').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#Distributor').getViewModel().setData(responseData.response.data);
     },
